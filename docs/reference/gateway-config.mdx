...
# proxy URLs are supported, in scheme://host:port form: a URL carrying a
# path, query, or fragment is rejected rather than silently truncated.
# NO_PROXY entries (hostnames, domain suffixes, IPs, CIDRs) are dialed
# directly. This is an operator-owned egress boundary:
...
# startup, setting no_proxy or proxy_auth_file without a proxy URL is
# rejected as well, and a set-but-invalid value reaching a sandbox (for
# example an unreadable or malformed auth file) is fatal to that sandbox's
# supervisor instead of silently falling back to direct or unauthenticated
# egress.
#
